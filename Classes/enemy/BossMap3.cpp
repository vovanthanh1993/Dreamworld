...
bool BossMap3::init(Vec2 position) {
    ...
    // Pool for bat
    batPool->createPool(world, scene, bodyToSpriteMap, 10);
...
void BossMap3::castShadow() {
    ...
    auto blackLayer = LayerColor::create(cocos2d::Color4B(0, 0, 0, 0), origin.x + Director::getInstance()->getVisibleSize().width, origin.y +Director::getInstance()->getVisibleSize().height);
